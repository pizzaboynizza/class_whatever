...
with open('The_War_of_the_Worlds.txt', 'r', encoding='utf-8') as file:
    contents_of_book = file.read()

def count_words(contents_of_book):
    translator = str.maketrans('', '', string.punctuation)
    string_without_punct = contents_of_book.translate(translator)
    print(string_without_punct)
    words = list(stopwords.items())
    print(words)
    ...
    # Your dictionary will have words as keys and counts as values. If a word isnt in your dictionary yet, add it with a count of 1. If it is, increment its count.
    # Print the most frequent top 10 out with their counts. You can do that with the code below.
    # word_dict is a dictionary where the key is the word and the value is the count

count_words(contents_of_book)
