...
# open the file
with open("project_gutenburg.txt", "r", encoding="utf8") as project_gutenburg:
    contents = project_gutenburg.read()
    # print(contents)
    ...
    # print(lower)
    sentence = lower.count(".")
    # print(sentence)


    translator = str.maketrans('', '', string.punctuation)
    string_without_punct = lower.translate(translator)
    # print(string_without_punct)

    # .split() says splits every word in string_without_punct by whitespace with a comma and " "
    new = string_without_punct.split()
    # print(new)
    ...
    # need to check to see if in dictionary if it is dont add if it is not add word
    # then add one to the count if word in dictionary.
